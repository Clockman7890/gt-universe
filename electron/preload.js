'use strict';
const { contextBridge, ipcRenderer } = require('electron');

contextBridge.exposeInMainWorld('gt', {
  listSaves:  ()        => ipcRenderer.invoke('save:list'),
  newCareer:  (slot, p) => ipcRenderer.invoke('career:new', slot, p),
  loadCareer: slot      => ipcRenderer.invoke('career:load', slot),
  deleteSlot: slot      => ipcRenderer.invoke('career:delete', slot),
  state:      ()        => ipcRenderer.invoke('career:state'),
  advance:    ()        => ipcRenderer.invoke('career:advance'),
  closeCareer:()        => ipcRenderer.invoke('career:close'),
  market:     ()        => ipcRenderer.invoke('market:list'),
  buyCar:     (m, l)    => ipcRenderer.invoke('market:buy', m, l),
  buyCars:    (m, ids)  => ipcRenderer.invoke('market:buyMany', m, ids),
  ownedCars:  ()        => ipcRenderer.invoke('market:owned'),
  enterCar:   (c, l)    => ipcRenderer.invoke('market:enter', c, l),
  usedCars:   ()        => ipcRenderer.invoke('market:used'),
  buyUsed:    (c, l)    => ipcRenderer.invoke('market:buyUsed', c, l),
  sellQuote:  c         => ipcRenderer.invoke('market:sellQuote', c),
  sellCar:    c         => ipcRenderer.invoke('market:sell', c),
  fixQuote:   c         => ipcRenderer.invoke('market:fixQuote', c),
  fixEngine:  c         => ipcRenderer.invoke('market:fix', c),
  carImage:   name      => ipcRenderer.invoke('market:image', name),
  carSpecs:   name      => ipcRenderer.invoke('car:specs', name),
  gameOverSound: ()     => ipcRenderer.invoke('sound:gameOver'),
  solvency:   ()        => ipcRenderer.invoke('money:solvency'),
  garage:     ()        => ipcRenderer.invoke('garage:list'),
  office:     ()        => ipcRenderer.invoke('office:list'),
  eligible:   ()        => ipcRenderer.invoke('office:eligible'),
  chooseChamp:id        => ipcRenderer.invoke('office:choose', id),
  facilitiesDue:()      => ipcRenderer.invoke('office:facDue'),
  upgradeFacilities:()  => ipcRenderer.invoke('office:facUp'),
  takeSeat:   id        => ipcRenderer.invoke('office:takeSeat', id),
  formTeam:   (lvl, nm) => ipcRenderer.invoke('office:formTeam', lvl, nm),
  signDriver: (d, e)    => ipcRenderer.invoke('office:sign', d, e),
  myEntries:  ()        => ipcRenderer.invoke('entries:mine'),
  lineup:     ()        => ipcRenderer.invoke('lineup:get'),
  setDriver:  (e, r, d) => ipcRenderer.invoke('lineup:set', e, r, d),
  home:       ()        => ipcRenderer.invoke('home:info'),
  setTutorial:n         => ipcRenderer.invoke('tutorial:set', n),
  raceInfo:   ()        => ipcRenderer.invoke('race:info'),
  racePrepare:leg       => ipcRenderer.invoke('race:prepare', leg),
  raceSheet:  (r, l)    => ipcRenderer.invoke('race:sheet', r, l),
  raceCost:   ()        => ipcRenderer.invoke('race:cost'),
  withdraw:   (rd, en)  => ipcRenderer.invoke('race:withdraw', rd, en),
  simulate:   (rd, lg)  => ipcRenderer.invoke('race:simulate', rd, lg),
  mySponsors: ()        => ipcRenderer.invoke('sponsors:mine'),
  readGame:   ()        => ipcRenderer.invoke('sm:read'),
  gameStatus: ()        => ipcRenderer.invoke('sm:status'),
  raceSave:   (l, e)    => ipcRenderer.invoke('race:save', l, e),
  writeFiles: files     => ipcRenderer.invoke('race:write', files),
  ams2Path:   ()        => ipcRenderer.invoke('ams2:path'),
  settings:   ()        => ipcRenderer.invoke('settings:get'),
  pickAiFolder:()       => ipcRenderer.invoke('settings:pickAiFolder'),
  worldTree:  ()        => ipcRenderer.invoke('world:tree'),
  standings:  (id, k)   => ipcRenderer.invoke('world:standings', id, k),
  calendar:   id        => ipcRenderer.invoke('world:calendar', id),
  news:       ()        => ipcRenderer.invoke('news:list'),
  markRead:   ()        => ipcRenderer.invoke('news:read'),
  quit:       ()        => ipcRenderer.invoke('app:quit'),
  paths:      ()        => ipcRenderer.invoke('app:paths')
});
